...
데이터 흐름(요약)
    1. ``find_embedding_candidates``: 소스 아이템과 임베딩 종류가 맞는 청크끼리 pgvector 유사도 상위 k 후보.
    ...
    4. ``sync_relation_catalog_from_llm_edges``: 허용 코드·sanitize 규칙으로 ``relation_kind`` /
       ``relation_topic_parent`` / ``relation_subtopic`` / ``relation_type`` 만 갱신(MVP: 엣지 테이블 없음).

의존성
    ``psycopg``, OpenAI 클라이언트 등 **무거운 의존성**은 ``entry``·``persist``·``llm_propose`` 쪽에만 두고,
...
def __getattr__(name: str) -> Any:
    """패키지 속성 지연 로드: ``import src.relations as R; R.propose_relations_for_item`` 형태만 지원."""
    if name == "propose_relations_for_item":
        from src.relations.entry import propose_relations_for_item as fn

        return fn
    if name == "find_embedding_candidates":
        from src.relations.candidates import find_embedding_candidates as fn
