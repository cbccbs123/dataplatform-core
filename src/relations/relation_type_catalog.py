...
def fetch_relation_kind_id(conn: Connection[Any], *, kind_code: str) -> int | None:
    """``kind_code`` 로 ``relation_kind_id`` 조회. 없으면 None."""
    with conn.cursor(row_factory=dict_row) as cur:
        cur.execute(
            """
    ...
    if row is None:
        return None
    return int(row["relation_kind_id"])
...
def ensure_relation_kind_for_llm_proposal(
    ...
    *,
    ...
    """
    LLM이 제안한 **새 kind_code** 를 ``relation_kind`` 에 반영(또는 기존 행 보강).

    - 신규: ``INSERT`` (보통 ``status='inactive'`` — 검토 전).
    ...
    with conn.cursor(row_factory=dict_row) as cur:
        ...
        return int(row2["relation_kind_id"])
...
def ensure_relation_topic_parent_row(
    ...
    *,
    ...
    """
    대주제 (``topic_ko``, ``topic_en``) 행을 보장하고 ``topic_id`` 를 반환한다.
    """
...
def ensure_relation_subtopic_row(
    ...
    *,
    ...
    """
    ``topic_id`` 아래 (``subtopic_ko``, ``subtopic_en``) 행을 보장하고 ``subtopic_id`` 를 반환한다.
    """
...
def ensure_relation_subtopic_bundle(
    ...
    *,
    ...
    """
    LLM 토피 네 필드에 대응하는 ``relation_subtopic.subtopic_id`` 를 보장·반환한다.
...
def fetch_relation_type_id(
    ...
    *,
    ...
    relation_subtopic_id: int,
    status: str | None = "active",
) -> int | None:
    """
    (``relation_kind_id``, ``relation_subtopic_id``) 에 해당하는 ``relation_type_id``.
    ...
    if row is None:
        return None
    return int(row["relation_type_id"])
...
def fetch_relation_type_id_for_normalized_edge(
    ...
    *,
    ...
    """
    정규화된 토피 번들 + ``kind_code`` 로 ``relation_type_id`` 를 찾는다.
    ...
    with conn.cursor(row_factory=dict_row) as cur:
        ...
        if row is not None:
            cur.execute(
                ...
                """,
                (status, int(row["relation_type_id"])),
            )
            return
        cur.execute(
            """
