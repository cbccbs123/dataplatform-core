...
# 상대 신호 임계: keep = (top − baseline) ≥ EPS. baseline 정의가 '전체 평균'→'하위 절반 평균'으로
# 바뀌어(밀집 토픽 '충전' 오컷 구조 해결) EPS 는 G4 에서 재보정한다. 현재는 운영 보정 잠정치(0.17).
OS_CUTOFF_EPS_DEFAULT: float = 0.17
...
# BM25 multi_match operator 기본값 — 'or'(현행). 'and' 는 전 토큰 매칭 강제(F2 복합어 가짜매칭 차단) 옵트인.
OS_BM25_OPERATOR_DEFAULT: str = "or"
