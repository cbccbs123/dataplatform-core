...
직접 import(테스트 patch 정본=``src.app.portal.routes_assets.<name>``), 인프라(``_run_in_db``)는 ``_infra``
...
def bundle(
    ...
    한계(MVP): 이웃 자산의 의료 필터는 미적용(현재 medical 자산 부재·plan §0.1) — 후속(013/RBAC)에서
    ``collect_bundle_assets`` 수준에 추가.
    """
