...
def backfill_assets(
    ...
    *,
    ...
    Returns:
        요약 카운트 dict — ``{scanned, classified, skipped_existing, no_text, failed, os_synced}``.
        ``no_text`` = 분류가 None(미부여)을 반환한 자산(대부분 자기 텍스트 없음 — 정밀 사유
        구분은 ``scripts/topic_grounding_report.py`` 후분석 몫). ``failed`` = 자산 처리 중 예외(격리).
    """
    ...
    for i, aid in enumerate(asset_ids, 1):
        ...
        try:
            ...
            def _txn(conn, _aid=aid):
                ...
                return classify_fn(conn, _aid, settings=settings, client=client)
    ...
    *,
    ...
    with db:
        if asset_ids:
            ...
            skip_existing = only_missing  # 지정 자산 재실행 멱등은 루프에서 확인
        else:
        ...
        summary = backfill_assets(
            db, targets, skip_existing=skip_existing,
