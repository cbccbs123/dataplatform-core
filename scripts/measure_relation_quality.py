...
PROMPT_VARIANTS: dict[str, dict] = {
    ...
    "baseline": {},
    # v2 후보 — 채택본(2026-07-29)의 "거의 같은 형식" 문구가 DB 정의("내용/주제/장면의 근접
    # 유사")보다 좁아 **매체만 다른 같은-대상 쌍**을 same_domain 으로 밀어냄이 골든 측정으로
    # 실증됐다(kind 오답 방향 역전 · 보고서 §9). 대상 동일성만 남기고 형식 조건을 제거한다.
    # 검증 통과 시에만 운영 힌트로 반영(다른 문구는 채택본 그대로).
    "dup-same-subject-v2": {
        "kind_hints_override": {
