...
class TestRecordResolutionIsolation(unittest.TestCase):
    ...
    def test_edges_zero_writes_pending_isolated_reason(self) -> None:
        db = _FakeDB(attempts={})
        ...
        self.assertEqual((aid, status, attempts), (_A1, "pending", 1))
...
class TestRunRelationsQueueWiring(unittest.TestCase):
    ...
    def test_three_cases_queue_status(self) -> None:
        ...
        self.assertEqual(by_aid[_A1][1:3], ("pending", 1))   # 고립
        self.assertEqual(by_aid[_A2][1:3], ("pending", 1))   # 예외
...
class TestResolutionPersistDB(unittest.TestCase):
    ...
    def test_decide_status_matches_persisted_flow(self) -> None:
        """순수 함수 결정과 실 DB 영속 흐름이 일치(엣지0→pending·증분)."""
        with self.db.transaction() as conn:
            ...
            upsert_resolution(conn, aid, status=status, attempts=attempts, reason="isolated:no_edges")
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT status, attempts FROM relation_resolution WHERE asset_id=%s", (aid,)
                )
                self.assertEqual(tuple(cur.fetchone()), ("pending", 1))
