...
class TestSnapshotBucketsE2E(unittest.TestCase):
    ...
    def test_by_snapshot_bucket_sum_equals_total_and_5buckets(self):
        from src.portal.asset_stats import _SNAPSHOT_BUCKETS, asset_stats
        db = _conn_ctx()
        with db:
    ...
    def test_relation_proposed_registered_partition(self):
        ...
        from src.portal.asset_stats import query_assets
        db = _conn_ctx()
        with db:
            with db.transaction() as conn:
    ...
    def test_timeline_month_group_by_modality(self):
        from src.portal.asset_stats import asset_timeline
        db = _conn_ctx()
        with db:
            with db.transaction() as conn:
